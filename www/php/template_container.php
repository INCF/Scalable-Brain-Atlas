...
$last_modified_template = filemtime($templatePath.'svgpaths.json');
...
if (!$svgCapable) {
  ...
  echo 'window.BRAIN_REGIONS='; readfile($templatePath.'brainregions.json'); echo ';'; 
  echo 'window.BRAIN_SLICES='; readfile($templatePath.'brainslices.json'); echo ';';
...
if (!isset($last_modified_cache) || $last_modified_cache<$last_modified_template) {
  ...
  $page[] = '//<![CDATA[';
  $page[] = 'window.SVG_PATHS='.file_get_contents($templatePath.'svgpaths.json').';'; 
  $page[] = 'window.BRAIN_REGIONS='.file_get_contents($templatePath.'brainregions.json').';'; 
  $page[] = 'window.BRAIN_SLICES='.file_get_contents($templatePath.'brainslices.json').';';
