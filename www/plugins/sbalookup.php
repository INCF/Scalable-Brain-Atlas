...
function update_cache() {
  ...
  if ($modified) {
    ...
    foreach ($templates as $template) {
      if (file_exists('../'.$template.'/template/acr2full.json')) {
  ...
  foreach ($templates as $template) {
    if (file_exists('../'.$template.'/template/acr2full.json')) {
      $s = file_get_contents('../'.$template.'/template/acr2full.json');
      ...
      if (file_exists('../'.$template.'/template/alias2acr.json')) {
        $s = file_get_contents('../'.$template.'/template/alias2acr.json');
    ...
    $s = file_get_contents('../'.$template.'/template/config.json');
