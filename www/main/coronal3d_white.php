...
class pluginField_class extends textField_class {
  function parseAndValidate(&$s) {
    if (is_string($s) && strlen($s)>0) {
      if (strncmp($s,'http://',7)==0 || strncmp($s,'https://',8)==0) {
        $ok = preg_match('/([\d\w_]+)_plugin\.js/',$s,$matches);
        if ($ok) {
          session_start();
          $approved = @$_SESSION['APPROVED_EXTERNAL_PLUGINS'][$s];
          if (!$approved) {
            return 'approve';
          } else {
            return;
          ...
          return 'External plugin should end with [lowercase_plugin_name]_plugin.js';
        }
      } else {
        // internal plugin
      }
...
$f = new pluginField_class('External plugin');
$applet->addFormField('plugin',$f);

$errors = $applet->parseAndValidateInputs($_REQUEST);
/*
if ($errors['plugin'] == 'approve') {
  echo '<html><head><script type="text/javascript">';
  echo '</script></head><body>';
  echo 'You are calling the Scalable Brain Atlas (SBA) with a non-empty plugin parameter.<br/>';
  echo 'If you continue, SBA will load javascript code from the following URL:<br/>';
  echo htmlspecialchars($externalPlugin).'</br>';
  echo 'Do not continue if you don\'t know or trust the authors of this plugin.<br/>';
  echo '<form action=""><input name="cancel" type="button" value="Cancel"/>&nbsp;<input name="continue" type="button" value="Continue, I understand the risk"/></form><br/>';
  echo 'Your choice will be stored in a cookie until you close your browser.<br/>';
  echo '<iframe name="callback" id="callback"></iframe></body></html>';
  exit(0);
...
<svg xmlns="http://www.w3.org/2000/svg">
  <defs>
    ...
    path.marker { opacity: 1; fill: #0F0; stroke: #000; stroke-width: 0.2in; }
    ...
    .hull_highlight { fill-opacity: 0.65; fill: #AAA; stroke-opacity: 1; stroke: #00F; stroke-width: 2.0in; }
    ]]>
    </style>
  </defs>
</svg>
...
if (isset($featuredPlugin)) {
  if (strncmp($featuredPlugin,'http://',7)==0 || strncmp($featuredPlugin,'https://',8)==0) {
    ...
    if ($ok) {
      ...
      session_start();
      $approved = @$_SESSION['APPROVED_EXTERNAL_PLUGINS'][$externalPlugin];
      if (!$approved) {
      }
...
?>
  <br/>
  Slice spacing:<br/>
  <select id="sliceSpacing3d" onchange="sbaViewer.selectSpacing(this.options[this.selectedIndex].value)"><option value="0">Fiducial</option><option value="1">Fit width</option><option value="2">Double width</option></select>
  <br/>
  Rotate view:<br/>
