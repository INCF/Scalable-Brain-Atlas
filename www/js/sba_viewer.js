...
sbaViewer_class.prototype.setState = function(state) {
  if (state.acr) this.currentAcr = state.acr;
...
sbaViewer_class.prototype.initPlugins = function(plugins,featuredPlugin) {
  ...
  for (var k in plugins) if (plugins.hasOwnProperty(k)) {
...
sbaViewer_class.prototype.showView3d = function(angle,modeSpacing3d) {
  ...
  if (this.svgCapable) {
    ...
    img = '<img alt="3D view, template '+this.template+'" src="../thumbnail.php?template='+this.template+'&size=L&dim=3d"/>';
    elem.innerHTML = img;
  }
...
sbaViewer_class.prototype.showRegion3D = function(acr,resultClass) {
  ...
  if (this.svgCapable) {
    ...
    img = '<img alt="3D view, region '+escB(acr)+', template '+this.template+'" src="../thumbnail.php?template='+this.template+'&region='+escB(acr)+'&size=L&dim=3d"/>';
    elem.innerHTML = img;
  }
}
...
sbaViewer_class.prototype.addMarkerElem = function(elem0,marker,dim) {
  ...
  if (markerShape == 'crosshair') {
    elem2.setAttribute('d',marker.crosshairPath());
    markerStyle.push('stroke: #000');
  } else {
  ...
  if (dim == 31) {
    ...
    if (markerShape == 'crosshair') {
      ...
      elem2a.setAttribute('style','stroke: '+marker.rgb);
...
sbaViewer_class.prototype.mouseoverRegion = function(ev,rgb) {
  ...
  if (rgb != undefined) {
    var acr = RGB_TO_ACR[rgb];
    tip += (acr == undefined ? '#'+rgb : BS_SUGGESTIONS[acr].replace(/ /g,'&#160;'))+(this.debug ? ' '+rgb+' '+ev.target.id : '')+'<br/>';
  }
  var xyPix = tooltip.elemXY(ev,'view2d_svg');
  var xySvg = this.view2d_pix2svg(xyPix);
  if (this.debug) {
    //tip += '<span style="font-size: small">XZ[%] ('+Math.round(1000*xyPixFrac[0])+','+' '+Math.round(1000*xyPixFrac[1])+')</span><br/>';
    //tip += '<span style="font-size: small">XZ[svg] ('+Math.round(xySvg[0])+','+' '+Math.round(xySvg[1])+')</span><br/>';
  }
...
sbaViewer_class.prototype.addOverlay2D = function(os) {
  ...
  if (parts) {
    ...
    if (parts[1].charAt(1) == '0') {
      ...
      var slice_no = '00000'.substr(0,zeropad-String(os).length)+os;
    }
    parts[1] = slice_no;
    if (this.svgCapable) {
      var svgElem = document.getElementById('VIEW2D_SVG_ELEM');
...
sbaViewer_class.prototype.showView2d = function(os,acr) {
  ...
  if (this.svgCapable) {
    ...
    if (this.overlayMode < 2) {
      elem.innerHTML = '<img alt="2D view, region '+escB(acr)+', template '+this.template+'" src="../thumbnail.php?template='+this.template+'&region='+escB(acr)+'&size=L&dim=2d"/>';
    } else {
      elem.innerHTML = '';
    }
  }
...
sbaViewer_class.prototype.regionCenter = function(acr) {
  var rgbList = this.getRgbList(acr);
  if (rgbList.length > 0 && window.RGB_CENTERS != undefined && window.RGB_VOLUMES != undefined) {
    var iLargest = 0;
    var vLargest = 0;
    ...
    if (ctr != undefined) {
      var xy = [ctr[1],ctr[2]];
      var origSlice = this.nearestOrigSlice(SLICE_POS[ctr[0]-1]);
      return [origSlice,xy];
    }
  }
}
