...
function sbaViewer_class(config,svgCapable) {
  ...
  // . SVG bounding box
  this.aspectRatio2d = 44/36;
  this.aspectRatio3d = 74/22;
  ...
  bb[1] -= 0.04*bb[3];
  bb[3] *= 1.08;
  this.sliceAspectRatio = bb[2]/bb[3];
  if (bb[2]<bb[3]*this.aspectRatio2d) {
    // narrow slices use more width to fit view2d panel
    bb[2] = Math.round(bb[3]*this.aspectRatio2d)
  }
  this.boundingBox = bb;
  ...
  this.defaultSliceSpacing = this.aspectRatio3d*svgHeight/(this.numSlices-1+10);
  
  // default slice spacing: fit in available viewport width
  this.bothHemispheres = config.hemisphere ? config.hemisphere == 'LR' : this.sliceAspectRatio>=0.9;
  var relWidthSlice = this.sliceAspectRatio*(this.bothHemispheres ? 0.5 : 1);
  relWidthSlice *= (config.sliceSpacing == undefined ? 105 : config.sliceSpacing)/100;
  this.sineAngle = 10/(this.numSlices-1+10)*this.aspectRatio3d/relWidthSlice;
...
sbaViewer_class.prototype.view3d_innerHTML = function(angle,modeSpacing3d) {
  ...
  var bb3d = [bb[0], bb[1]-2*topMargin, Math.round(bb[3]*10*this.aspectRatio3d/(this.numSlices-1+10)+(this.numSlices-1)*sliceSpacing), bb[3]+2*topMargin];

  a.push('<svg id="VIEW3D_SVG_ELEM" class="slice3d" xmlns="'+this.nameSpace+'" xmlns:xlink="http://www.w3.org/1999/xlink" xml:space="preserve" preserveAspectRatio="xMinYMin meet" style="shape-rendering:geometricPrecision; text-rendering:geometricPrecision; fill-rule:evenodd" width="100%" viewBox="'+bb3d.join(' ')+'">');
...
sbaViewer_class.prototype.addOverlay2D = function(os) {
  ...
  if (parts) {
    ...
    if (slice_no) {
      ...
      if (this.svgCapable) {
        var svgElem = document.getElementById('VIEW2D_SVG_ELEM');
        // var pctWidth = 100*svgElem.width.baseVal.value // percentage
        var pctWidth = 100
        var bb = this.boundingBox
        ...
        var w = pctWidth/bb[2]*anchorBox[2]
        pctWidthPerSvg = pctWidth/bb[2]
        var actualAspectRatio2d = imgElem.offsetParent.offsetWidth/imgElem.offsetParent.offsetHeight
        var moveRight = (anchorBox[0]-bb[0])*pctWidthPerSvg
        var moveDown = (anchorBox[1]-bb[1])*actualAspectRatio2d*pctWidthPerSvg
        imgElem.style.width = ''+w+'%'
        imgElem.style.left = moveRight+'%'
        imgElem.style.top = moveDown+'%'
      }
...
sbaViewer_class.prototype.view2d_pix2svg = function(xyPix) {
  ...
  var xyPixFrac = [(xyPix[0]+0.4999)/ctm.a,(xyPix[1]+0.4999)/ctm.d];
  return [bb[0]+xyPixFrac[0],bb[1]+xyPixFrac[1]*bb[3]];
}
...
sbaViewer_class.prototype.selectSpacing = function(mode) {
  ...
  if (elem) {
    ...
    elem.width.baseVal.value = sliceSpacing/this.defaultSliceSpacing;
  }
